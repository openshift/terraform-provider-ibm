		...
		Refresh: func() (interface{}, string, error) {
			...
			if getErr != nil {
				if apiErr, ok := getErr.(bmxerror.RequestFailure); ok {
					...
					return nil, "", fmt.Errorf("Reading volume attach %s failed with resp code: %s, err: %v", d.Id(), volume, getErr)
				}
				return nil, "", fmt.Errorf("Reading volume attach failed: %s", getErr)
			}
