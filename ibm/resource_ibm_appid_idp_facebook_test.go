...
func TestAccAppIDIDPFacebook_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: setupIBMAppIDFacebookIDPConfig(appIDTenantID),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ibm_appid_idp_facebook.fb", "tenant_id", appIDTenantID),
...
func testAccCheckIBMAppIDIDPFacebookDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err != nil {
			return fmt.Errorf("Error checking if AppID Facebook IDP configuration was reset: %s", err)
		}

		if config == nil || (config.IsActive != nil && *config.IsActive != false) {
			return fmt.Errorf("Error checking if AppID Facebook IDP configuration was reset")
		}
	}
