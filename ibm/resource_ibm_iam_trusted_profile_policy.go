...
func resourceIBMIAMTrustedProfilePolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if _, ok := d.GetOk("resources"); ok {
		d.Set("resources", flattenPolicyResource(trustedProfilePolicy.Resources))
	}
	if _, ok := d.GetOk("resource_attributes"); ok {
		d.Set("resource_attributes", flattenPolicyResourceAttributes(trustedProfilePolicy.Resources))
	}
	if len(trustedProfilePolicy.Resources) > 0 {
		if *getResourceAttribute("serviceType", trustedProfilePolicy.Resources[0]) == "service" {
			d.Set("account_management", false)
		}
		if *getResourceAttribute("serviceType", trustedProfilePolicy.Resources[0]) == "platform_service" {
...
func importTrustedProfilePolicy(d *schema.ResourceData, meta interface{}) (interface{}, interface{}, error) {
	...
	resources := flattenPolicyResource(trustedProfilePolicy.Resources)
	resource_attributes := flattenPolicyResourceAttributes(trustedProfilePolicy.Resources)
