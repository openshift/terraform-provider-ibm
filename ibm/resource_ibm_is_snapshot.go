	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			isSnapshotName: {
				...
				Optional:     true,
				Computed:     true,
				ValidateFunc: InvokeValidator("ibm_is_snapshot", isSnapshotName),
...
func isSnapshotRefreshFunc(sess *vpcv1.VpcV1, id string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		if err != nil {
			return nil, isSnapshotFailed, fmt.Errorf("Error getting Snapshot : %s\n%s", err, response)
		}
...
func snapshotDelete(d *schema.ResourceData, meta interface{}, id string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Error getting Snapshot (%s): %s\n%s", id, err, response)
	}
	...
	if err != nil {
		return fmt.Errorf("Error deleting Snapshot : %s\n%s", err, response)
	}
...
func isSnapshotDeleteRefreshFunc(sess *vpcv1.VpcV1, id string) resource.StateRefreshFunc {
	return func() (interface{}, string, error) {
		...
		if err != nil {
			...
			return nil, isSnapshotFailed, fmt.Errorf("The Snapshot %s failed to delete: %s\n%s", id, err, response)
		}
		return snapshot, *snapshot.LifecycleState, nil
	}
}
...
func snapshotExists(d *schema.ResourceData, meta interface{}, id string) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf("Error getting Snapshot: %s\n%s", err, response)
	}
	return true, nil
}
