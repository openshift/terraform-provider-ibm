...
func testAccCheckIbmSatelliteEndpointExists(n string, obj satellitelinkv1.Endpoint) resource.TestCheckFunc {
	...
	return func(s *terraform.State) error {
		...
		satelliteLinkClient, err := testAccProvider.Meta().(ClientSession).SatellitLinkClientSession()
		if err != nil {
			return err
		}
...
func testAccCheckIbmSatelliteEndpointDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err == nil {
			...
			return fmt.Errorf("Error checking for satellite_endpoint (%s) has been destroyed: %s", rs.Primary.ID, err)
		}
	}
