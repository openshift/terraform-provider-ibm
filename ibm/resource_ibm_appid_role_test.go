...
func TestAccIBMAppIDRole_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMAppIDRoleConfig(appIDTenantID, roleName, appName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ibm_appid_role.test_role", "tenant_id", appIDTenantID),
...
func testAccCheckIBMAppIDRoleDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err == nil {
			return fmt.Errorf("error checking if AppID role (%s) has been destroyed", rs.Primary.ID)
		}
	}
