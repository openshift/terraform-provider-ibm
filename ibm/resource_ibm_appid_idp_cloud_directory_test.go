...
func TestAccIBMAppIDIDPCloudDirectory_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMAppIDIDPCloudDirectoryConfig(appIDTenantID),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ibm_appid_idp_cloud_directory.idp", "tenant_id", appIDTenantID),
...
func testAccCheckIBMAppIDIDPCloudDirectoryDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err != nil {
			return fmt.Errorf("Error checking if AppID IDP Cloud Directory configuration was reset: %s", err)
		}
		...
		if config == nil || diff != "" {
			return fmt.Errorf("Error checking if AppID IDP Cloud Directory configuration was reset: %s", diff)
		}
	}
