...
func TestAccIBMAppIDIDPSaml_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckIBMAppIDIDPSAMLConfig(appIDTenantID, dispName),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ibm_appid_idp_saml.test_saml", "tenant_id", appIDTenantID),
...
func testAccCheckIBMAppIDIDPSAMLDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err != nil {
			return fmt.Errorf("Error checking if AppID IDP SAML configuration was reset: %s", err)
		}
		...
		if config == nil || diff != "" {
			return fmt.Errorf("Error checking if AppID IDP SAML configuration was reset: %s", diff)
		}
	}
